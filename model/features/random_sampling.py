...
def determine_num_crops(depressed_dict, normal_dict, crop_width=125):
    ...
    """
    # merged_dict = dict(normal_dict, **depressed_dict) k
    merged_dict = dict()
    # merged_dict['normal'] = normal_dict
    # merged_dict['depressed'] = depressed_dict
    for ket in normal_dict.keys():
        merged_dict[ket] = normal_dict[ket]
    for ket in depressed_dict.keys():
        merged_dict[ket] = depressed_dict[ket]
...
def get_random_samples(matrix, n_samples, crop_width):
    ...
    print(len(cropped_sample_ls), math.ceil(n_samples))

    # get random samples
    samples = random.sample(cropped_sample_ls, math.floor(n_samples))
    return samples
...
def rand_samp_train_test_split(npz_file_dir):
    ...
    for sample in norm_select_samps[:-num_test_samples]:
        ...
        with np.load(npz_file) as data:
            for key in data.keys():
                train_samples.append(data[key])
    train_labels = np.concatenate((np.ones(math.ceil(len(train_samples)/2), dtype=int),
                                   np.zeros(math.ceil(len(train_samples)/2), dtype=int)))
    ...
    for sample in norm_select_samps[-num_test_samples:]:
        ...
        with np.load(npz_file) as data:
            for key in data.keys():
                test_samples.append(data[key])
    test_labels = np.concatenate((np.ones(math.ceil(len(test_samples)/2), dtype=int),
                                  np.zeros(math.ceil(len(test_samples)/2), dtype=int)))
